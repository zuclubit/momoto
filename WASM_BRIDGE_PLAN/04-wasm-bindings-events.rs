...
// Provides event bus, streaming, and subscription system for JS consumers.
// =============================================================================
...
use momoto_events::{
    event::{
        ...
        EventPayload as CoreEventPayload,
        ProgressEvent as CoreProgressEvent,
        MetricEvent as CoreMetricEvent,
        RecommendationEvent as CoreRecommendationEvent,
        ValidationEvent as CoreValidationEvent,
        ChartUpdateEvent as CoreChartUpdateEvent,
        ChartDataPoint as CoreChartDataPoint,
        HeartbeatEvent as CoreHeartbeatEvent,
        SystemEvent as CoreSystemEvent,
    },
    broadcaster::{
...
impl MomotoEventBus {
    ...
    /// The callback receives event JSON as a string.
    #[wasm_bindgen]
    pub fn subscribe(&self, callback: js_sys::Function) -> Result<u64, JsValue> {
        ...
        // Safety: JS functions are Send+Sync in wasm-bindgen context
        ...
        let id = subscription.id().0; // Extract internal ID
        std::mem::forget(subscription); // Keep subscription alive
        Ok(id)
    }

    /// Subscribe with category filter.
    /// categories: array of category numbers (0=Progress, 1=Metrics, etc.)
...
impl MomotoEventStream {
    ...
    /// Poll for available events. Returns JSON of BatchedEvents or null.
    #[wasm_bindgen]
    ...
    /// Check if flush should happen (batch full or timeout).
    ...
    /// Current stream state: "Active", "Paused", "Closed".
    #[wasm_bindgen(getter)]
