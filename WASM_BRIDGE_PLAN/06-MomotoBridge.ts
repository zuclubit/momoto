...
export const MomotoBridge = {
  ...
  // ---------------------------------------------------------------------------
  // OKLab (NEW — delegates to Rust momoto-core::OKLab)
  // ---------------------------------------------------------------------------
  ...
  contrast: {
    ...
    wcag(fg: InstanceType<WasmModule['Color']>, bg: InstanceType<WasmModule['Color']>) {
      const metric = wasm().WCAGMetric.new();
    ...
    apca(fg: InstanceType<WasmModule['Color']>, bg: InstanceType<WasmModule['Color']>) {
      const metric = wasm().APCAMetric.new();
      const result = metric.evaluate(fg, bg);
      metric.free();
      return result;
    },
  ...
  quality: {
    ...
    score(
      ...
      const ctx = wasm().RecommendationContext.new(usage, target);
      const scorer = wasm().QualityScorer.new();
  ...
  events: {
    ...
    createBus(bufferSize = 100, maxAgeMs = 30000) {
      return wasm().MomotoEventBus.withConfig(bufferSize, maxAgeMs);
    },
    ...
    createBatchedStream(
      ...
      return wasm().MomotoEventStream.fromBusBatched(bus, batchSize, timeoutMs);
    },
  },

  // ---------------------------------------------------------------------------
  // Glass Materials (already in momoto-wasm, thin re-export)
  // ---------------------------------------------------------------------------
  ...
  css: {
    ...
    render(material: InstanceType<WasmModule['GlassMaterial']>, context?: InstanceType<WasmModule['RenderContext']>) {
      ...
      const evalCtx = wasm().EvalMaterialContext.default();
  ...
  refraction: {
    ...
    /** Calculate refraction at position. Returns { offsetX, offsetY, hueShift, brightness }. */
    calculate(params: InstanceType<WasmModule['RefractionParams']>, x: number, y: number) {
      return wasm().calculateRefraction(params, x, y);
    },

    /** Apply refraction to OKLCH. Returns { l, c, h }. */
    applyToColor(
      params: InstanceType<WasmModule['RefractionParams']>,
      l: number, c: number, h: number,
      x: number, y: number,
    ) {
      return wasm().applyRefractionToColor(params, l, c, h, x, y);
    },
  ...
  lighting: {
    /** Presets */
    studio: () => wasm().LightingEnvironment.studio(),
    dramatic: () => wasm().LightingEnvironment.dramatic(),
    ambient: () => wasm().LightingEnvironment.ambient(),
    ...
    calculate(
      ...
      return wasm().calculateLighting(env, normalX, normalY, normalZ, roughness);
    },
    ...
    gradient(
      ...
      return wasm().deriveGradient(env, samples, roughness);
    },

    /** Gradient to CSS linear-gradient string. */
    gradientCss(
      env: InstanceType<WasmModule['LightingEnvironment']>,
      samples: number,
      roughness: number,
      ...
      return wasm().gradientToCss(env, samples, roughness, angleDeg);
    },
  },
  ...
  shadows: {
    ...
    /** Calculate ambient shadow CSS. */
    ambient(params: InstanceType<WasmModule['AmbientShadowParams']>, elevation: number): string {
      return wasm().calculateAmbientShadow(params, elevation);
    },

    /** Multi-scale ambient shadow CSS (3 layers). */
    multiScale(params: InstanceType<WasmModule['AmbientShadowParams']>, elevation: number): string {
      return wasm().calculateMultiScaleAmbient(params, elevation);
    },
    ...
    interactive(
      ...
      params: InstanceType<WasmModule['AmbientShadowParams']>,
    ): string {
      return wasm().calculateInteractiveShadow(transition, state, params);
    },
  ...
  pbr: {
    ...
    soapBubble: () => wasm().ThinFilmBSDF.soapBubble(),
    oilSlick: () => wasm().ThinFilmBSDF.oilSlick(),

    /** Create layered material stack. */
    layered: () => new (wasm().LayeredBSDF)(),

    /** Create Lambertian (diffuse) BSDF. */
    lambertian: (r: number, g: number, b: number) => new (wasm().LambertianBSDF)(r, g, b),
    ...
    evaluateBatch(iors: Float64Array, roughnesses: Float64Array, opacities: Float64Array) {
      return wasm().evaluateMaterialBatch(iors, roughnesses, opacities);
    },
  },

  // ---------------------------------------------------------------------------
  // Spectral Coherence (delegates to Rust momoto-materials::spectral_coherence)
  // ---------------------------------------------------------------------------

  spectral: {
    /** Spectrally coherent interpolation (no hue shift artifacts). */
    interpolate(
      ...
      const result = new Uint8Array(
        wasm().spectralInterpolate(fromR, fromG, fromB, toR, toG, toB, t, true)
      );
      return [result[0]!, result[1]!, result[2]!];
    },

    /** Generate spectrally coherent gradient. Returns Uint8Array [r,g,b,...]. */
    gradient(
      fromR: number, fromG: number, fromB: number,
      toR: number, toG: number, toB: number,
      steps: number,
      ...
      return new Uint8Array(
        wasm().spectralGradient(fromR, fromG, fromB, toR, toG, toB, steps)
      );
    },

    /** Flicker detection validator. */
    flickerValidator: (threshold?: number) =>
      threshold
        ? wasm().FlickerValidator.withThreshold(threshold)
        : new (wasm().FlickerValidator)(),
  },
  ...
  temporal: {
    /** Temporal interpolation modes. */
    interpolation: {
      linear: () => wasm().Interpolation.linear(),
      smooth: () => wasm().Interpolation.smooth(),
      spring: () => wasm().Interpolation.spring(),
    },

    /** Rate limiter for smooth transitions. */
    rateLimiter: (maxChangePerSec: number) => new (wasm().RateLimiter)(maxChangePerSec),

    /** Exponential moving average. */
    ema: (alpha: number) => new (wasm().ExponentialMovingAverage)(alpha),

    /** Time-varying dielectric. */
    dielectric: (baseIor: number, baseRoughness: number, iorRate: number, roughnessRate: number) =>
      new (wasm().TemporalDielectric)(baseIor, baseRoughness, iorRate, roughnessRate),

    /** Time-varying thin film. */
    thinFilm: (baseThickness: number, amplitude: number, frequency: number) =>
      new (wasm().TemporalThinFilm)(baseThickness, amplitude, frequency),

    /** Time-varying conductor. */
    conductor: (baseTemp: number, heatingRate: number) =>
      new (wasm().TemporalConductor)(baseTemp, heatingRate),
  },
  ...
  constraints: {
    ...
    validator: (tolerance?: number) =>
      tolerance
        ? wasm().ConstraintValidator.withTolerance(tolerance)
        : new (wasm().ConstraintValidator)(),
  },
  ...
  deltaE: {
    ...
    labToRgb: (l: number, a: number, b: number): [number, number, number] => {
      const rgb = new Uint8Array(wasm().labToRgb(l, a, b));
  ...
  cssEnhanced: {
    /** Render enhanced CSS with all effects. */
    render(material: InstanceType<WasmModule['EvaluatedMaterial']>, context?: InstanceType<WasmModule['RenderContext']>): string {
      const ctx = context ?? wasm().RenderContext.desktop();
      return wasm().renderEnhancedCss(material, ctx);
    },

    /** Render premium CSS with animations. */
    renderPremium(material: InstanceType<WasmModule['EvaluatedMaterial']>, context?: InstanceType<WasmModule['RenderContext']>): string {
      const ctx = context ?? wasm().RenderContext.desktop();
      return wasm().renderPremiumCss(material, ctx);
    },
  },
  ...
  presets: {
    ...
    byQuality: () => wasm().getPresetsByQuality(),
  },
} as const;

// =============================================================================
// Type Definitions (for TS consumers)
// =============================================================================
