# Momoto v5.0: A Technical Whitepaper
...
This whitepaper presents Momoto v5.0, a TypeScript library for perceptual color management in design systems. We evaluate the library against industry standards and comparable tools (Material Color Utilities, culori, Color.js, apca-w3) across five dimensions: accuracy, accessibility, performance, decision capabilities, and developer experience.
...
**Conclusion:** Momoto v5.0 is suitable for high-performance color operations and design token generation. However, the APCA contrast calculation should not be used for accessibility compliance validation until accuracy deviations are resolved.
...
This whitepaper evaluates Momoto v5.0's capabilities in:
...
|---------|-------------|---------------|-------------|
| Momoto | 1.4M ops/s | 33% pass | ~25KB |
...
Momoto achieves performance parity with the fastest available color libraries. Key factors:
...
**Recommendation:** Until resolved, do not use Momoto for accessibility compliance decisions. Use apca-w3 or bridge to reference implementation.
...
Momoto v5.0 provides a well-architected, high-performance color management solution for TypeScript design systems. The library excels at:
...
```typescript
// Core Value Objects
import { OKLCH, HCT, APCAContrast } from 'momoto';
...
// Accessibility validation
import { validateColorPair } from 'momoto';
...
# Run benchmark suite
npx vitest run src/lib/momoto/__tests__/benchmark.test.ts

# Or run directly
npx ts-node src/lib/momoto/benchmark/index.ts
```
