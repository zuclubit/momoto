# @zuclubit/momoto-ui — Golden Paths & Anti-Patterns Catalog

> **Version:** 1.0.0
> **Philosophy:** Make the right thing easy, make the wrong thing hard.

---

## 🎯 Purpose

Este documento define:
- **Golden Paths:** La forma correcta y fácil de hacer las cosas
- **Anti-Patterns:** Errores comunes que deben evitarse
- **Early Warning Signs:** Señales de que algo va mal
- **Recovery Patterns:** Cómo corregir errores

---

## 🏆 GOLDEN PATHS

### GP-001: Setup Inicial Correcto

**Objetivo:** Configurar un proyecto nuevo en menos de 30 minutos

```typescript
// ✅ GOLDEN PATH: Setup completo en root

// 1. app/providers.tsx
import {
  ThemeProvider,
  GovernanceProvider,
  ENTERPRISE_POLICIES
} from '@zuclubit/momoto-ui';

export function Providers({ children }: { children: React.ReactNode }) {
  return (
    <GovernanceProvider
      policies={ENTERPRISE_POLICIES.STANDARD}
      enforceMode="strict"
      onViolation={(v) => console.warn('momoto-ui violation:', v)}
    >
      <ThemeProvider defaultTheme="system" storageKey="ui-theme">
        {children}
      </ThemeProvider>
    </GovernanceProvider>
  );
}

// 2. app/layout.tsx
import { Providers } from './providers';

export default function RootLayout({ children }) {
  return (
    <html>
      <body>
        <Providers>{children}</Providers>
      </body>
    </html>
  );
}
```

**Verificación:**
```bash
npx @zuclubit/momoto-ui doctor
# ✓ GovernanceProvider detected at root
# ✓ ThemeProvider detected
# ✓ ENTERPRISE_POLICIES.STANDARD loaded
# ✓ Ready for development
```

---

### GP-002: Uso de Colores Semánticos

**Objetivo:** Nunca usar colores hardcodeados en componentes

```typescript
// ✅ GOLDEN PATH: Colores semánticos via CSS variables

// Component using semantic tokens
function StatusBadge({ status }: { status: 'success' | 'error' | 'warning' }) {
  return (
    <span
      className={cn(
        'px-2 py-1 rounded-full text-sm',
        {
          'bg-semantic-success/10 text-semantic-success': status === 'success',
          'bg-semantic-error/10 text-semantic-error': status === 'error',
          'bg-semantic-warning/10 text-semantic-warning': status === 'warning',
        }
      )}
    >
      {status}
    </span>
  );
}

// Tailwind config (auto-generated by momoto-ui)
// tailwind.config.ts
import { getTailwindConfig } from '@zuclubit/momoto-ui/adapters/tailwind';

export default getTailwindConfig({
  // Tokens already included
});
```

**Verificación:**
```bash
npx @zuclubit/momoto-ui lint --no-hardcoded-colors
# ✓ 0 hardcoded colors found
```

---

### GP-003: Accesibilidad desde el Diseño

**Objetivo:** Garantizar contraste accesible automáticamente

```typescript
// ✅ GOLDEN PATH: Usar funciones de contraste del momoto-ui

import { getOptimalForeground, validateContrast } from '@zuclubit/momoto-ui';

function Card({ bgColor, children }: { bgColor: string; children: React.ReactNode }) {
  // Automáticamente selecciona el mejor foreground
  const textColor = getOptimalForeground(bgColor, {
    standard: 'apca-body', // Lc 60+ para texto de cuerpo
  });

  // Validación en desarrollo
  if (process.env.NODE_ENV === 'development') {
    const { valid, ratio } = validateContrast(bgColor, textColor);
    if (!valid) {
      console.warn(`Contrast issue: ${ratio} (required: 60 Lc)`);
    }
  }

  return (
    <div style={{ backgroundColor: bgColor, color: textColor }}>
      {children}
    </div>
  );
}
```

**Verificación:**
```bash
npx @zuclubit/momoto-ui accessibility --standard=apca-body
# ✓ All text meets Lc 60+ requirement
```

---

### GP-004: Tokens de Componente

**Objetivo:** Crear tokens de componente que heredan del sistema

```typescript
// ✅ GOLDEN PATH: Tokens de componente derivados

// components/Button/Button.tokens.ts
import { deriveComponentTokens } from '@zuclubit/momoto-ui';

export const buttonTokens = deriveComponentTokens('button', {
  base: {
    // Heredan de tokens semánticos
    background: 'var(--color-primary)',
    foreground: 'var(--color-primary-foreground)',
    border: 'var(--color-primary-border)',
    // Valores específicos de componente
    borderRadius: 'var(--radius-md)',
    paddingX: 'var(--spacing-4)',
    paddingY: 'var(--spacing-2)',
  },
  variants: {
    secondary: {
      background: 'var(--color-secondary)',
      foreground: 'var(--color-secondary-foreground)',
    },
    destructive: {
      background: 'var(--color-destructive)',
      foreground: 'var(--color-destructive-foreground)',
    },
  },
  states: {
    hover: {
      background: 'var(--color-primary-hover)',
    },
    disabled: {
      opacity: '0.5',
      cursor: 'not-allowed',
    },
  },
});

// Auto-registra en el token collection
// Auto-genera CSS variables
// Auto-valida accesibilidad
```

---

### GP-005: Theming Multi-tenant

**Objetivo:** Soportar branding por tenant sin duplicar código

```typescript
// ✅ GOLDEN PATH: Theming dinámico

import { ThemeProvider, useTenantTheme } from '@zuclubit/momoto-ui';

function TenantApp({ tenantId }: { tenantId: string }) {
  // Carga tema del tenant desde backend/config
  const { theme, isLoading } = useTenantTheme(tenantId);

  if (isLoading) return <Skeleton />;

  return (
    <ThemeProvider
      theme={theme}
      overrides={{
        // Solo overrides específicos del tenant
        colors: {
          primary: theme.brandColor,
        },
      }}
    >
      <App />
    </ThemeProvider>
  );
}

// Los componentes NO necesitan cambios
// El tema se aplica via CSS variables
```

---

### GP-006: Migración de Legacy

**Objetivo:** Migrar código legacy sin breaking changes

```typescript
// ✅ GOLDEN PATH: Migración gradual con coexistencia

// Phase 1: Wrapper de coexistencia
import { LegacyBridge } from '@zuclubit/momoto-ui/migration';

function LegacyModule() {
  return (
    <LegacyBridge
      tokenMapping={{
        // Mapea tokens legacy a nuevos
        '--old-primary': 'var(--color-primary)',
        '--old-text': 'var(--color-foreground)',
      }}
      validateOnRender={process.env.NODE_ENV === 'development'}
    >
      <OldComponent />
    </LegacyBridge>
  );
}

// Phase 2: Reemplazo gradual
// npx @zuclubit/momoto-ui migrate --dry-run
// npx @zuclubit/momoto-ui migrate --apply

// Phase 3: Remover LegacyBridge
// Eliminar cuando todos los tokens estén migrados
```

---

### GP-007: Validación en Development

**Objetivo:** Detectar problemas antes del commit

```typescript
// ✅ GOLDEN PATH: Hooks de desarrollo

// hooks/useDevValidation.ts
import { useLiveConformance, useComponentAudit } from '@zuclubit/momoto-ui';

export function useDevValidation() {
  // Solo en desarrollo
  if (process.env.NODE_ENV !== 'development') {
    return { violations: [], warnings: [] };
  }

  // Validación en tiempo real
  const conformance = useLiveConformance();
  const audit = useComponentAudit();

  // Mostrar overlay de desarrollo
  useEffect(() => {
    if (conformance.violations.length > 0) {
      showDevOverlay(conformance.violations);
    }
  }, [conformance.violations]);

  return { violations: conformance.violations, warnings: audit.warnings };
}
```

```typescript
// Configurar DevTools
// momoto-ui.config.ts
export default {
  devtools: {
    enabled: process.env.NODE_ENV === 'development',
    showViolations: true,
    highlightIssues: true,
    logToConsole: true,
    overlay: {
      position: 'bottom-right',
      maxHeight: '300px',
    },
  },
};
```

---

### GP-008: Reportes Automáticos

**Objetivo:** Generar reportes sin intervención manual

```yaml
# ✅ GOLDEN PATH: CI/CD con reportes

# .github/workflows/momoto-ui-report.yml
- name: Generate Quality Report
  run: |
    npx @zuclubit/momoto-ui report \
      --format=markdown \
      --output=QUALITY_REPORT.md

- name: Update PR with Report
  uses: actions/github-script@v7
  with:
    script: |
      const fs = require('fs');
      const report = fs.readFileSync('QUALITY_REPORT.md', 'utf8');
      github.rest.issues.createComment({
        issue_number: context.issue.number,
        owner: context.repo.owner,
        repo: context.repo.repo,
        body: report
      });
```

---

## 🚫 ANTI-PATTERNS

### AP-001: Colores Hardcodeados

**Descripción:** Usar valores hex/rgb directamente en componentes

```typescript
// ❌ ANTI-PATTERN: Hardcoded colors
function BadButton() {
  return (
    <button
      style={{
        backgroundColor: '#3b82f6', // ❌ Hardcoded
        color: 'white',             // ❌ Hardcoded
        border: '1px solid #2563eb' // ❌ Hardcoded
      }}
    >
      Click me
    </button>
  );
}

// ❌ También en Tailwind
function AlsoBad() {
  return (
    <button className="bg-[#3b82f6] text-white border-[#2563eb]">
      Click me
    </button>
  );
}
```

**Por qué es malo:**
- Rompe theming (dark mode, tenants)
- No es auditable
- Inconsistencia visual
- Imposible mantener

**Cómo detectar:**
```bash
npx @zuclubit/momoto-ui lint --no-hardcoded-colors
# ✗ 3 hardcoded colors found
#   src/components/Button.tsx:12 - #3b82f6
#   src/components/Button.tsx:13 - white
#   src/components/Button.tsx:14 - #2563eb
```

**Cómo corregir:**
```typescript
// ✅ CORRECT: Use semantic tokens
function GoodButton() {
  return (
    <button className="bg-primary text-primary-foreground border-primary-border">
      Click me
    </button>
  );
}
```

---

### AP-002: Bypass de Governance

**Descripción:** Desactivar governance para "ir más rápido"

```typescript
// ❌ ANTI-PATTERN: Disabling governance
<GovernanceProvider
  policies={ENTERPRISE_POLICIES.STANDARD}
  enforceMode="off"  // ❌ NUNCA hacer esto
>
  <App />
</GovernanceProvider>

// ❌ También via env vars
// .env
UI_KIT_GOVERNANCE=disabled  // ❌ No existe, no funcionará

// ❌ Intentar bypass con wrapper
function UnsafeWrapper({ children }) {
  // ❌ Esto no hace nada, governance es heredada
  return <div data-no-governance>{children}</div>;
}
```

**Por qué es malo:**
- Deuda técnica instantánea
- Violaciones se acumulan
- Corregir después es 10x más caro
- Rompe consistencia cross-product

**Cómo detectar:**
```bash
npx @zuclubit/momoto-ui check-provider
# ✗ GovernanceProvider found with enforceMode="off"
# ✗ This is a policy violation
```

**Cómo corregir:**
```typescript
// ✅ CORRECT: Use warn mode during development
<GovernanceProvider
  policies={ENTERPRISE_POLICIES.STANDARD}
  enforceMode={process.env.NODE_ENV === 'development' ? 'warn' : 'strict'}
>
  <App />
</GovernanceProvider>
```

---

### AP-003: Tokens Sin Registrar

**Descripción:** Crear tokens custom sin registrarlos

```typescript
// ❌ ANTI-PATTERN: Unregistered tokens
const customStyles = {
  '--my-custom-color': '#ff0000',  // ❌ No registrado
  '--my-spacing': '24px',          // ❌ No registrado
};

function BadComponent() {
  return (
    <div style={customStyles}>
      <p style={{ color: 'var(--my-custom-color)' }}>
        Text
      </p>
    </div>
  );
}
```

**Por qué es malo:**
- No aparece en auditorías
- No se valida accesibilidad
- No se exporta a Figma/Style Dictionary
- Inconsistencia

**Cómo detectar:**
```bash
npx @zuclubit/momoto-ui lint --no-unregistered-tokens
# ✗ 2 unregistered tokens found
#   --my-custom-color (src/styles.ts:2)
#   --my-spacing (src/styles.ts:3)
```

**Cómo corregir:**
```typescript
// ✅ CORRECT: Register tokens properly
import { registerProductTokens } from '@zuclubit/momoto-ui';

// Register at app init
registerProductTokens('my-product', {
  colors: {
    'custom-accent': {
      value: '#ff0000',
      description: 'Accent color for highlights',
      // Automáticamente validado
    },
  },
  spacing: {
    'custom-lg': {
      value: '24px',
      description: 'Large custom spacing',
    },
  },
});

// Use registered tokens
function GoodComponent() {
  return (
    <div className="text-custom-accent spacing-custom-lg">
      Text
    </div>
  );
}
```

---

### AP-004: Ignorar Warnings de Accesibilidad

**Descripción:** Marcar warnings como "false positive" sin investigar

```typescript
// ❌ ANTI-PATTERN: Suppressing a11y warnings
function BadCard() {
  return (
    // ❌ Suppressing without understanding
    // @ts-ignore: A11Y_CONTRAST_LOW
    <div className="bg-gray-100 text-gray-300">
      Low contrast text
    </div>
  );
}

// ❌ También en config
// momoto-ui.config.ts
export default {
  accessibility: {
    ignoreWarnings: ['contrast-low'],  // ❌ NUNCA
  },
};
```

**Por qué es malo:**
- Usuarios con discapacidad visual no pueden usar
- Posible violación legal (ADA, EAA)
- UX degradado para todos en condiciones adversas

**Cómo detectar:**
```bash
npx @zuclubit/momoto-ui accessibility --verbose
# ⚠️ 5 warnings suppressed via comments/config
# This is a policy violation
```

**Cómo corregir:**
```typescript
// ✅ CORRECT: Fix the actual issue
import { getOptimalForeground } from '@zuclubit/momoto-ui';

function GoodCard() {
  const bgColor = 'var(--color-muted)';
  const textColor = getOptimalForeground(bgColor, { standard: 'wcag-aa' });

  return (
    <div style={{ backgroundColor: bgColor, color: textColor }}>
      Accessible text
    </div>
  );
}
```

---

### AP-005: Copy-Paste de Componentes momoto-ui

**Descripción:** Copiar código de componentes en lugar de importarlos

```typescript
// ❌ ANTI-PATTERN: Copy-pasting component code
// components/Button.tsx
// "Copied from momoto-ui and modified"
export function Button({ variant = 'default', ...props }) {
  // ... 200 lines of copied code
  // ... with "small modifications"
}

// ❌ Ahora tienes:
// - Duplicación
// - No recibe updates
// - Posibles bugs introducidos
// - Sin validación de governance
```

**Por qué es malo:**
- Divergencia inevitable
- No recibe bug fixes
- Sin governance
- Mantenimiento doble

**Cómo detectar:**
```bash
npx @zuclubit/momoto-ui detect-duplicates
# ⚠️ Potential duplicate of @zuclubit/momoto-ui/Button
#    src/components/Button.tsx (87% similarity)
```

**Cómo corregir:**
```typescript
// ✅ CORRECT: Extend via composition
import { Button as BaseButton } from '@zuclubit/momoto-ui';

// Extend with custom variants
export function Button({ variant, ...props }) {
  const customVariants = {
    'brand-special': 'bg-brand-accent text-white',
  };

  return (
    <BaseButton
      className={customVariants[variant]}
      {...props}
    />
  );
}

// ✅ Or use the slots pattern
import { Button } from '@zuclubit/momoto-ui';

<Button
  slots={{
    root: ({ children }) => <div className="custom-wrapper">{children}</div>,
  }}
>
  Click me
</Button>
```

---

### AP-006: Múltiples ThemeProviders

**Descripción:** Anidar múltiples providers de tema

```typescript
// ❌ ANTI-PATTERN: Nested theme providers
function App() {
  return (
    <ThemeProvider theme="light">
      <Header />
      <ThemeProvider theme="dark">  {/* ❌ Nested! */}
        <Sidebar />
      </ThemeProvider>
      <ThemeProvider theme="light">  {/* ❌ Another nested! */}
        <Content />
      </ThemeProvider>
    </ThemeProvider>
  );
}
```

**Por qué es malo:**
- Performance overhead
- Comportamiento impredecible
- CSS variable conflicts
- Debugging nightmare

**Cómo detectar:**
```bash
npx @zuclubit/momoto-ui check-provider --detect-nesting
# ✗ Multiple ThemeProvider instances detected
#   Root: app/layout.tsx:15
#   Nested: components/Sidebar.tsx:8
#   Nested: components/Content.tsx:12
```

**Cómo corregir:**
```typescript
// ✅ CORRECT: Single provider, component-level overrides
function App() {
  return (
    <ThemeProvider theme="light">
      <Header />
      <Sidebar data-theme="dark" />  {/* ✅ Override via data attr */}
      <Content />
    </ThemeProvider>
  );
}

// CSS handles the override
// [data-theme="dark"] uses dark CSS variables
```

---

## ⚠️ EARLY WARNING SIGNS

### EWS-001: Conformance Score Dropping

| Signal | Meaning | Action |
|--------|---------|--------|
| Score drops 5+ points in 1 week | New violations introduced | Review recent PRs |
| Score plateau below 75 | Technical debt accumulating | Sprint allocation needed |
| Score variance > 10 between modules | Inconsistent adoption | Module-specific training |

### EWS-002: Token Coverage Declining

| Signal | Meaning | Action |
|--------|---------|--------|
| Coverage drops 3+ points | New hardcoded values | Run lint on recent PRs |
| Custom tokens growing fast | Possible divergence | Review token registration |
| Unused tokens > 20% | Bloated token set | Deprecation review |

### EWS-003: Exception Rate Rising

| Signal | Meaning | Action |
|--------|---------|--------|
| > 5 exceptions in 1 week | Policies too strict OR bypassed | Policy review meeting |
| Same exception repeated | Systemic issue | Update policy or training |
| Exceptions not expiring | Tracking failure | Audit exception registry |

### EWS-004: Build Failure Rate Spiking

| Signal | Meaning | Action |
|--------|---------|--------|
| > 10% builds fail momoto-ui | Major breaking change | Emergency review |
| Same failure repeating | Knowledge gap | Targeted training |
| Failures only in one team | Adoption issue | Team-specific support |

---

## 🔄 RECOVERY PATTERNS

### RP-001: Recovering from Low Conformance

```bash
# 1. Identify top violators
npx @zuclubit/momoto-ui audit --top-violations=10

# 2. Prioritize by impact
npx @zuclubit/momoto-ui triage --severity=critical

# 3. Auto-fix what's possible
npx @zuclubit/momoto-ui fix --auto --dry-run
npx @zuclubit/momoto-ui fix --auto --apply

# 4. Create tickets for manual fixes
npx @zuclubit/momoto-ui export-issues --format=jira
```

### RP-002: Recovering from Token Drift

```bash
# 1. Detect all unregistered tokens
npx @zuclubit/momoto-ui token-audit --unregistered

# 2. Generate registration code
npx @zuclubit/momoto-ui token-register --generate

# 3. Map hardcoded to semantic
npx @zuclubit/momoto-ui token-migrate --interactive
```

### RP-003: Recovering from Accessibility Violations

```bash
# 1. Full accessibility audit
npx @zuclubit/momoto-ui accessibility --full --output=a11y-audit.json

# 2. Generate fix suggestions
npx @zuclubit/momoto-ui accessibility --suggest-fixes

# 3. Apply auto-fixable issues
npx @zuclubit/momoto-ui accessibility --fix --apply
```

---

## 📋 Quick Reference Card

### ✅ Always Do

| Practice | Command to Verify |
|----------|-------------------|
| Use GovernanceProvider | `npx @zuclubit/momoto-ui check-provider` |
| Use semantic tokens | `npx @zuclubit/momoto-ui lint --no-hardcoded` |
| Register custom tokens | `npx @zuclubit/momoto-ui token-audit` |
| Fix a11y issues immediately | `npx @zuclubit/momoto-ui accessibility` |
| Run conformance before PR | `npx @zuclubit/momoto-ui conformance` |

### ❌ Never Do

| Anti-Pattern | Detection |
|--------------|-----------|
| Hardcode colors | `--no-hardcoded-colors` |
| Bypass governance | `check-provider` |
| Ignore a11y warnings | `accessibility --verbose` |
| Copy component code | `detect-duplicates` |
| Nest ThemeProviders | `check-provider --detect-nesting` |

---

> **Regla de Oro:** Si tienes que "hacer un workaround" para algo del momoto-ui, probablemente estás haciendo algo mal. Pregunta primero.
