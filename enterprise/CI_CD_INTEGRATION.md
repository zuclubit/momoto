# @zuclubit/momoto-ui — CI/CD Integration Model
...
Integrar `@zuclubit/momoto-ui` como **quality gate obligatorio** en todos los pipelines, asegurando que:
...
│       ▼             ▼             ▼             ▼             ▼             │
│  ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐      │
│  │ ESLint  │   │  Unit   │   │ momoto-ui  │   │  WCAG   │   │ Bundle  │      │
...
```yaml
# .github/workflows/momoto-ui-conformance.yml
- name: momoto-ui Conformance Check
  run: |
    npx @zuclubit/momoto-ui conformance \
      --min-score=75 \
      --fail-on-regression \
...
- name: Accessibility Audit
  run: |
    npx @zuclubit/momoto-ui accessibility \
      --standard=wcag-aa \
      --fail-on-critical \
...
- name: Token Coverage Analysis
  run: |
    npx @zuclubit/momoto-ui token-coverage \
      --min-coverage=80 \
      --no-hardcoded \
...
- name: Policy Compliance Check
  run: |
    npx @zuclubit/momoto-ui policy-check \
      --policy-set=standard \
      --strict \
...
```yaml
# .github/workflows/momoto-ui-quality-gate.yml
name: momoto-ui Quality Gate
...
jobs:
  ...
  quick-checks:
    ...
    steps:
      ...
      - name: Lint momoto-ui Usage
        run: npx @zuclubit/momoto-ui lint --fix=false

      - name: Check Governance Provider
        run: npx @zuclubit/momoto-ui check-provider
  ...
  conformance:
    ...
    steps:
      ...
      - name: Run Conformance Analysis
        id: conformance
        run: |
          RESULT=$(npx @zuclubit/momoto-ui conformance \
            --min-score=75 \
            --json \
            --output=conformance-report.json)
  ...
  accessibility:
    ...
    steps:
      ...
      - name: Run Accessibility Audit
        id: a11y
        run: |
          RESULT=$(npx @zuclubit/momoto-ui accessibility \
            --standard=wcag-aa \
            --json \
            --output=a11y-report.json)
  ...
  token-coverage:
    ...
    steps:
      ...
      - name: Analyze Token Usage
        id: tokens
        run: |
          RESULT=$(npx @zuclubit/momoto-ui token-coverage \
            --json \
            --output=token-report.json)
      ...
      - name: Generate Summary
        ...
        with:
          script: |
            ...
            const summary = `
            ## 🎨 momoto-ui Quality Gate Results
            ...
            ---
            *Generated by @zuclubit/momoto-ui quality gate*
            `;
...
```json
{
  "required_status_checks": {
    "strict": true,
    "contexts": [
      "momoto-ui Quality Gate",
      "momoto-ui Conformance Check",
      "momoto-ui Accessibility Audit"
    ]
  },
...
- name: Export Prometheus Metrics
  run: |
    npx @zuclubit/momoto-ui metrics \
      --format=prometheus \
      --output=/tmp/metrics.prom

    # Push to Pushgateway
    curl --data-binary @/tmp/metrics.prom \
      http://pushgateway:9091/metrics/job/momoto-ui/instance/${{ github.repository }}
```
...
- name: Run with secrets
  run: |
    npx @zuclubit/momoto-ui validate 2>&1 | grep -v "license\|secret\|key"
```
