# @zuclubit/momoto-ui — Adoption Checklist

> **Instrucciones:** Completa esta checklist para adoptar el momoto-ui correctamente.
...
- [ ] Copié `momoto-ui.config.example.ts` a la raíz del proyecto
- [ ] Renombré a `momoto-ui.config.ts`
...
```typescript
// app/providers.tsx
import { ThemeProvider, GovernanceProvider, ENTERPRISE_POLICIES } from '@zuclubit/momoto-ui';
...
```bash
npx @zuclubit/momoto-ui validate
```
...
```bash
npx @zuclubit/momoto-ui audit --init
```
...
```bash
npx @zuclubit/momoto-ui lint --no-hardcoded-colors
```
...
```typescript
// tailwind.config.ts
import { getTailwindConfig } from '@zuclubit/momoto-ui/adapters/tailwind';
...
```bash
npx @zuclubit/momoto-ui accessibility --standard=wcag-aa
```
...
```bash
npx @zuclubit/momoto-ui conformance --suggest-improvements
```
...
```bash
npx @zuclubit/momoto-ui validate --full
```
...
1. **Self-service:** `npx @zuclubit/momoto-ui doctor --verbose`
2. **Slack:** #momoto-ui-support
