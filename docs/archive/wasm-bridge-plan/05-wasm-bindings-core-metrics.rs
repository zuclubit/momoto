...
// Appended to existing momoto-wasm/src/lib.rs.
// =============================================================================

use wasm_bindgen::prelude::*;
use momoto_core::{
    color::Color as CoreColor,
    space::oklch::{OKLab as CoreOKLab, OKLCH as CoreOKLCH, HuePath as CoreHuePath},
    luminance::{
        self as lum,
    ...
    evaluated::{
        ...
        ClearCoatProperties as CoreClearCoat,
        IridescenceProperties as CoreIridescence,
        NoiseProperties as CoreNoise,
        MaterialType as CoreMaterialType,
    },
    render::{
        ColorSpace as CoreColorSpace,
        TargetMedium as CoreTargetMedium,
        AccessibilityMode as CoreAccessibilityMode,
    },
};
use momoto_metrics::{
    wcag::{
...
impl OKLab {
    ...
    /// Linear interpolation in OKLab space (better than OKLCH for some cases).
    #[wasm_bindgen]
    ...
    /// Euclidean distance in OKLab (perceptual color difference).
...
/// Calculate WCAG 2.1 relative luminance from a color hex.
...
/// Calculate APCA relative luminance from a color hex.
...
/// Inverse linear interpolation (value → t).
...
/// Determine the highest WCAG level achieved by a contrast ratio.
/// Returns 0 = Fail, 1 = AA, 2 = AAA.
...
/// font_weight: CSS font weight (100-900).
...
/// Returns the ratio threshold (e.g., 4.5 for AA Normal).
...
pub fn wcag_requirements_matrix() -> Box<[f64]> {
    Box::new([
        WCAG_REQUIREMENTS[0][0], // AA normal = 4.5
        WCAG_REQUIREMENTS[0][1], // AA large = 3.0
        WCAG_REQUIREMENTS[1][0], // AAA normal = 7.0
        WCAG_REQUIREMENTS[1][1], // AAA large = 4.5
    ])
}

/// Calculate WCAG contrast ratio directly from two colors.
/// Faster than full evaluate() when you only need the ratio.
...
// =============================================================================
// APCA Constants (algorithm reference values)
// =============================================================================

/// Get APCA algorithm constants as JSON.
/// Returns: { mainTrc, sRco, sGco, sBco, blkThrs, blkClmp, normBg, normTxt,
///            revBg, revTxt, scaleBow, scaleWob, loBowOffset, loWobOffset,
///            loClip, deltaYMin }
...
// =============================================================================
// CssRenderConfig (missing from current bindings)
// =============================================================================
...
impl CssRenderConfig {
    ...
    /// Minimal effects (performance-first).
    #[wasm_bindgen]
    pub fn minimal() -> Self {
        Self { inner: momoto_core::backend::css_config::CssRenderConfig::minimal() }
    }

    /// Premium glass effects (all features).
    #[wasm_bindgen]
    pub fn premium() -> Self {
    ...
    /// Export as JSON for debugging/serialization.
...
// =============================================================================
// Enums (missing from current bindings)
// =============================================================================
...
/// Check if an evaluated material is transparent (opacity < 1.0).
...
/// Get effective specular intensity (factoring roughness).
...
/// Input: Uint8Array of [r, g, b, r, g, b, ...]
/// Output: Float64Array of luminance values.
...
/// Input: Uint8Array of [fg_r, fg_g, fg_b, bg_r, bg_g, bg_b, ...]
/// Output: Float64Array of contrast ratios.
