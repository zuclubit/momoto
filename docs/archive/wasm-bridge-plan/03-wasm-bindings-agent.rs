...
// Provides query/response system, contract validation, workflow execution,
// experience generation, and certification authority.
// =============================================================================
...
use momoto_agent::{
    ...
    contract::{
        ...
        constraint::{
            ...
            ContrastStandard as CoreContrastStandard,
            ComplianceLevel as CoreComplianceLevel,
            Gamut as CoreGamut,
        },
    },
    executor::AgentExecutor as CoreAgentExecutor,
    query::Query as CoreQuery,
    response::{
        Response as CoreResponse,
        ValidationResponse as CoreValidationResponse,
        ColorMetrics as CoreColorMetrics,
    },
};
...
impl AgentExecutor {
    ...
    /// This is the universal entry point — accepts any query type.
    #[wasm_bindgen]
...
// =============================================================================
// Convenience functions (skip query serialization)
// =============================================================================

/// Validate a single color against a contract.
/// contract_json: JSON string of Contract.
/// Returns JSON string of ValidationResponse.
...
/// Returns JSON: { lightness, chroma, hue, relativeLuminance, contrastVsWhite, ... }
...
/// Returns JSON of MaterialResponse, or null if not found.
...
/// context: "body_text" | "large_text" | "interactive" | "decorative"
/// target: "wcag_aa" | "wcag_aaa" | "apca" | "hybrid"
...
/// Input: JSON array of { fg: string, bg: string, standard: number, level: number }
/// Output: JSON array of ValidationResponse.
...
pub fn agent_validate_pairs_batch(pairs_json: &str) -> Result<String, JsValue> {
    ...
    let results: Vec<CoreValidationResponse> = pairs.iter().map(|p| {
...
/// Input: JSON array of hex strings.
/// Output: JSON array of ColorMetrics.
...
pub fn agent_get_metrics_batch(colors_json: &str) -> Result<String, JsValue> {
    ...
    let results: Vec<CoreColorMetrics> = colors.iter()
...
// =============================================================================
// Contract Builder (fluent API for constraint construction)
// =============================================================================
...
impl ContractBuilder {
    ...
    /// Add minimum WCAG AA contrast constraint against a background color.
    #[wasm_bindgen(js_name = "minContrastWcagAA")]
    pub fn min_contrast_wcag_aa(mut self, against_hex: &str) -> Self {
        self.constraints.push(CoreConstraint::min_contrast_wcag_aa(against_hex));
        self
    }

    /// Add minimum WCAG AAA contrast constraint.
    #[wasm_bindgen(js_name = "minContrastWcagAAA")]
    pub fn min_contrast_wcag_aaa(mut self, against_hex: &str) -> Self {
        self.constraints.push(CoreConstraint::min_contrast_wcag_aaa(against_hex));
        self
    }

    /// Require color to be in sRGB gamut.
    #[wasm_bindgen(js_name = "inSrgb")]
    pub fn in_srgb(mut self) -> Self {
        self.constraints.push(CoreConstraint::in_gamut(CoreGamut::Srgb));
        self
    }

    /// Require color to be in Display P3 gamut.
    #[wasm_bindgen(js_name = "inP3")]
    pub fn in_p3(mut self) -> Self {
        self.constraints.push(CoreConstraint::in_gamut(CoreGamut::P3));
        self
    }

    /// Constrain OKLCH lightness range [min, max] (0.0-1.0).
    #[wasm_bindgen(js_name = "lightnessRange")]
    pub fn lightness_range(mut self, min: f32, max: f32) -> Self {
        self.constraints.push(CoreConstraint::lightness_range(min, max));
        self
    }

    /// Constrain OKLCH chroma range [min, max] (0.0-0.4).
    #[wasm_bindgen(js_name = "chromaRange")]
    pub fn chroma_range(mut self, min: f32, max: f32) -> Self {
        self.constraints.push(CoreConstraint::chroma_range(min, max));
        self
    }

    /// Constrain OKLCH hue range [min, max] (0-360).
    ...
    /// Build the contract and return it as JSON string.
    #[wasm_bindgen]
    ...
    /// Build and validate a color in a single call.
...
/// This is the highest-level API — produces full theme with accessibility,
/// materials, typography, and CSS output.
///
/// preset: "minimal" | "premium" | "glass" | "enterprise"
/// primary_hex: Brand primary color
/// background_hex: Theme background color
///
/// Returns JSON of VisualExperience.
...
pub fn generate_experience(
    ...
    // Use the agent executor with a GenerateExperience query
...
/// Returns JSON of ColorSequence with interpolated states.
...
pub fn create_color_transition(
    ...
    let from = Color::from_hex(from_hex)
        .map_err(|e| JsValue::from_str(&e))?;
    let to = Color::from_hex(to_hex)
        .map_err(|e| JsValue::from_str(&e))?;
...
/// Returns JSON with certification results.
