import { CssVariablesAdapter } from './chunk-MWLAVRNX.mjs';
import { failure } from './chunk-445P5ZF2.mjs';
import { createContext, useReducer, useRef, useEffect, useCallback, useMemo, useContext, useState } from 'react';
...
function ThemeProvider({
  ...
  const registeredThemesRef = useRef(/* @__PURE__ */ new Map());
  useEffect(() => {
  ...
  const applyTheme = useCallback(async (config) => {
  ...
  const registerTheme = useCallback(async (config) => {
    if (!adapterRef.current) {
      return failure(new Error("Theme adapter not initialized"));
    }
...
export { ThemeContext, ThemeProvider, useAppliedTokens, useDarkMode, useSystemPreferences, useTheme, useThemeContext, useThemePreferences, useThemeSwitcher, useThemeVariable };
//# sourceMappingURL=chunk-XY4XUXGJ.mjs.map
//# sourceMappingURL=chunk-XY4XUXGJ.mjs.map