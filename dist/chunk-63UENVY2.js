...
var W3CTokenExporter = class {
  ...
  async exportTo(collection, options, destination) {
    ...
    if (!exportResult.success) {
      return chunk5YMPXU57_js.failure(exportResult.error);
    }
    let location;
  ...
  validateOptions(options) {
    if (!options.format) {
      return chunk5YMPXU57_js.failure(new Error("Export format is required"));
    }
    if (!this.supportsFormat(options.format)) {
      return chunk5YMPXU57_js.failure(new Error(`Unsupported format: ${options.format}`));
    }
  ...
  async validateDestination(destination) {
    switch (destination.type) {
      case "file":
        if (!destination.path) {
          return chunk5YMPXU57_js.failure(new Error("File path is required"));
        }
        break;
      case "api":
        if (!destination.url) {
          return chunk5YMPXU57_js.failure(new Error("API URL is required"));
        }
        try {
          new URL(destination.url);
        } catch {
          return chunk5YMPXU57_js.failure(new Error("Invalid API URL"));
        }
        break;
      case "figma":
        if (!destination.fileKey) {
          return chunk5YMPXU57_js.failure(new Error("Figma file key is required"));
        }
        break;
    }
    return chunk5YMPXU57_js.success(void 0);
  ...
  async preview(collection, options, maxLines = 50) {
    ...
    return chunk5YMPXU57_js.success({ preview, truncated, totalLines });
  }
  // ─────────────────────────────────────────────────────────────────────────
  // BATCH OPERATIONS
  // ─────────────────────────────────────────────────────────────────────────
  async exportMultiple(collection, formats) {
    ...
    if (errors.length > 0 && results.size === 0) {
      return chunk5YMPXU57_js.failure(new Error(`All exports failed: ${errors.join("; ")}`));
    }
...
exports.W3CTokenExporter = W3CTokenExporter;
//# sourceMappingURL=chunk-63UENVY2.js.map
//# sourceMappingURL=chunk-63UENVY2.js.map