...
var GenerateComponentTokens = class {
  ...
  constructor(derivationService, tokenRepository) {
    ...
    // DEPENDENCIES
    // ─────────────────────────────────────────────────────────────────────────
    chunkZM4FIU5F_js.__publicField(this, "derivationService");
    chunkZM4FIU5F_js.__publicField(this, "tokenRepository");
    this.derivationService = derivationService || new chunkUQITNDIL_js.TokenDerivationService();
  ...
  async execute(input) {
    try {
      ...
      if (!validationResult.success) {
        return chunk5YMPXU57_js.failure(validationResult.error);
      }
      const colorResult = chunkX3KESCNX_js.PerceptualColor.tryFromHex(input.brandColorHex);
      if (!colorResult.success) {
        return chunk5YMPXU57_js.failure(new Error(`Invalid brand color: ${colorResult.error.message}`));
      }
      const brandColor = colorResult.value;
      const intentResult = chunkUQITNDIL_js.ComponentIntent.from(input.intent);
      if (!intentResult.success) {
        return chunk5YMPXU57_js.failure(new Error(`Invalid intent: ${intentResult.error.message}`));
      }
  ...
  validateInput(input) {
    if (!input.componentName || input.componentName.trim().length === 0) {
      return chunk5YMPXU57_js.failure(new Error("Component name is required"));
    }
    if (!input.brandColorHex || input.brandColorHex.trim().length === 0) {
      return chunk5YMPXU57_js.failure(new Error("Brand color hex is required"));
    }
    if (!/^#[0-9A-Fa-f]{6}$/.test(input.brandColorHex)) {
      return chunk5YMPXU57_js.failure(new Error("Brand color must be a valid 6-digit hex color"));
    }
    if (!input.intent || input.intent.trim().length === 0) {
      return chunk5YMPXU57_js.failure(new Error("Intent is required"));
    }
    return chunk5YMPXU57_js.success(void 0);
  }
  /**
...
var EvaluateComponentAccessibility = class {
  ...
  async execute(input) {
    try {
      ...
      if (pairs.length === 0) {
        return chunk5YMPXU57_js.failure(new Error("No color pairs to evaluate"));
      }
      const results = [];
      const violations = [];
      ...
      if (this.auditPort && results.length > 0) {
        for (const result of results) {
          const fgResult = chunkX3KESCNX_js.PerceptualColor.tryFromHex(result.foreground);
          const bgResult = chunkX3KESCNX_js.PerceptualColor.tryFromHex(result.background);
  ...
  evaluatePair(pair, input) {
    const fgResult = chunkX3KESCNX_js.PerceptualColor.tryFromHex(pair.foreground);
    const bgResult = chunkX3KESCNX_js.PerceptualColor.tryFromHex(pair.background);
    if (!fgResult.success || !bgResult.success) {
  ...
  generateFixSuggestion(result) {
    const bgColor = chunkX3KESCNX_js.PerceptualColor.tryFromHex(result.background);
    const fgColor = chunkX3KESCNX_js.PerceptualColor.tryFromHex(result.foreground);
...
var ApplyPerceptualPolicy = class {
  ...
  async execute(input) {
    try {
      ...
      if (!collection || collection.all().length === 0) {
        return chunk5YMPXU57_js.failure(new Error("Token collection is empty"));
      }
      const violations = [];
      const colorTokens = collection.byType("color");
  ...
  createCorrectedCollection(original, violations) {
    ...
    return chunkUQITNDIL_js.TokenCollection.from(`${original.name}-corrected`, allTokens, original.description);
  }
  /**
...
var ExportDesignTokens = class {
  ...
  constructor(exporterPort, auditPort) {
    ...
    // DEPENDENCIES
    // ─────────────────────────────────────────────────────────────────────────
    chunkZM4FIU5F_js.__publicField(this, "exporterPort");
    chunkZM4FIU5F_js.__publicField(this, "auditPort");
  ...
  validateInput(input) {
    if (!input.collection) {
      return chunk5YMPXU57_js.failure(new Error("Token collection is required"));
    }
    if (!input.formats || input.formats.length === 0) {
      return chunk5YMPXU57_js.failure(new Error("At least one export format is required"));
    }
    for (const format of input.formats) {
      if (!this.exporterPort.supportsFormat(format)) {
        return chunk5YMPXU57_js.failure(new Error(`Unsupported export format: ${format}`));
      }
    }
    return chunk5YMPXU57_js.success(void 0);
  }
  ...
  applyFilterTransform(collection, config) {
    ...
    return chunkUQITNDIL_js.TokenCollection.from(`${collection.name}-filtered`, [...tokens], collection.description);
  }
  /**
...
var AuditVisualDecisions = class {
  ...
  validateInput(input) {
    if (!input.auditTypes || input.auditTypes.length === 0) {
      return chunk5YMPXU57_js.failure(new Error("At least one audit type is required"));
    }
    if (input.auditTypes.includes("token-usage") && !input.tokenCollection) {
      return chunk5YMPXU57_js.failure(new Error("Token collection is required for token-usage audit"));
    }
    return chunk5YMPXU57_js.success(void 0);
  }
  /**
...
exports.ApplyPerceptualPolicy = ApplyPerceptualPolicy;
exports.AuditVisualDecisions = AuditVisualDecisions;
exports.EXPORT_PRESETS = EXPORT_PRESETS;
exports.EvaluateComponentAccessibility = EvaluateComponentAccessibility;
exports.ExportDesignTokens = ExportDesignTokens;
exports.GenerateComponentTokens = GenerateComponentTokens;
exports.PRESET_POLICIES = PRESET_POLICIES;
//# sourceMappingURL=chunk-INDVKMSQ.js.map
//# sourceMappingURL=chunk-INDVKMSQ.js.map