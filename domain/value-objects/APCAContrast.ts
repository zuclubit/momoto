...
// WASM Integration - FASE 3 (2026-01-06)
// Feature flag for Rust/WASM backend (target: ≥6x performance improvement)
let wasmModule: typeof import('../../wasm/apca_wasm') | null = null;
let wasmInitialized = false;

// Feature flag: Enable WASM if environment variable is set
// Can be toggled at build time: ENABLE_WASM_APCA=true npm run build
const ENABLE_WASM =
...
async function initializeWasm(): Promise<void> {
  if (!ENABLE_WASM || wasmInitialized) return;

  try {
    wasmModule = await import('../../wasm/apca_wasm');
    wasmInitialized = true;
    console.log('[APCA] WASM backend initialized successfully');
  } catch (error) {
    console.warn('[APCA] WASM initialization failed, using TypeScript fallback:', error);
    wasmModule = null;
  }
}

// Try to initialize WASM immediately (async, non-blocking)
if (ENABLE_WASM) {
...
const APCA = {
  ...
  blkClmp: 1.414,
  // Output clipping threshold (loClip in APCA spec)
  // CRITICAL FIX (2026-01-06): Changed from 0.001 to 0.1 per canonical implementation
  // Reference: https://github.com/Myndex/apca-w3/blob/master/src/apca-w3.js
  loClip: 0.1,
  deltaYmin: 0.0005,
  normBG: 0.56,
  normTXT: 0.57,
  ...
  loBoWoffset: 0.027,
  loWoBoffset: 0.027,
  // FIX (2026-01-06): Removed loBoWthresh and loWoBthresh
  // These constants DO NOT EXIST in canonical APCA implementation
  // They were incorrectly invented and should not be used
} as const;
...
export class APCAContrast {
   ...
   *
   * FASE 3 Integration: Uses Rust/WASM backend if available (≥6x faster),
   * with automatic fallback to TypeScript implementation.
   */
  static calculate(textHex: string, bgHex: string): APCAContrast {
    // Try WASM backend first (if enabled and initialized)
    if (wasmModule && wasmInitialized) {
      try {
        ...
        if (textRgb && bgRgb) {
          ...
          const lc = wasmModule.apca_contrast(
            textRgb.r, textRgb.g, textRgb.b,
            bgRgb.r, bgRgb.g, bgRgb.b
          );

          // Determine polarity from Lc sign
          const polarity: APCAPolarity = lc >= 0 ? 'dark-on-light' : 'light-on-dark';

          return new APCAContrast(lc, polarity, textHex, bgHex);
        }
      } catch (error) {
        // WASM call failed, fall through to TypeScript
        console.warn('[APCA] WASM calculation failed, using TypeScript fallback:', error);
      }
    }
  ...
  // ============================================
  // WASM Integration Utilities (FASE 3)
  // ============================================

  /**
   * Check if WASM backend is enabled and initialized
   */
  static isUsingWasm(): boolean {
    return ENABLE_WASM && wasmInitialized && wasmModule !== null;
  }

  /**
   * Manually initialize WASM backend
   * Useful for testing or explicit initialization
   */
  static async initializeWasm(): Promise<boolean> {
    if (!ENABLE_WASM) {
      console.warn('[APCA] WASM is disabled via feature flag');
      return false;
    }
    ...
    try {
      ...
      console.error('[APCA] WASM initialization failed:', error);
      return false;
    }
  }

  /**
   * Get backend type currently in use
   */
  static getBackendType(): 'wasm' | 'typescript' {
    return APCAContrast.isUsingWasm() ? 'wasm' : 'typescript';
  }
...
/**
 * Convert hex color to APCA luminance (Y)
 *
 * NOTE: This uses simple gamma 2.4 transformation, NOT the standard sRGB piecewise function.
 * This appears to be intentional per APCA spec (to be verified against Myndex reference).
 *
 * The simple transformation is: linear = (sRGB / 255) ^ 2.4
 *
 * Standard sRGB uses piecewise transformation, but APCA may use simplified gamma.
 * Golden vectors (from Myndex SAPC-APCA) expect this simple transformation.
 *
 * See APCA_ACCURACY_ISSUE.md for investigation details.
 */
function hexToApcaLuminance(hex: string): number | null {
  const rgb = hexToRgb(hex);
  if (!rgb) return null;

  // APCA uses simple gamma transformation (not piecewise sRGB)
...
function calculateAPCA(
  ...
  // FIX (2026-01-06): Add deltaYmin check per canonical implementation
  // If luminance difference is too small, return zero contrast
  if (Math.abs(backY - textY) < APCA.deltaYmin) {
    return { lc: 0, polarity: 'dark-on-light' };
  }
  ...
  // Low contrast clamp
  // CRITICAL FIX (2026-01-06): Use loClip (0.1) instead of invented loBoWthresh/loWoBthresh
  // Following canonical Myndex implementation exactly
